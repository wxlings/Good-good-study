 ...
 **两者的区别在于:当数据不可写入的时候`getWritableDatabase()`会抛异常,而 `getReadableDatabase()`会以只读的方式打开**
 ...
 ```java
    ...
    // 通过传递一个上下文就可以访问数据库了
    SQLiteHelper(applicationContext).writableDatabase
...
```java
    ...
    SQLiteHelper(applicationContext).writableDatabase.insert("product",null,values)

```

##### updata()

使用方法`updata()`进行对已有数据进行更新
...
```java
    ...
    SQLiteHelper(applicationContext).writableDatabase.update("product",values,"id > ? AND stocks < ?", arrayOf("12","30"))
```
...
参数3: 需要更新的数据,顺序与参数3顺序相对应,均为String类型
